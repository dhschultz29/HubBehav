function refFuncDataStruct = takuReffunc_ModalityControl_v2(EDATImport, analysisName)
%Taku Ito
...
for sNum = 1:length(EDATDataStruct)
    ...
    disp(['Subject: ' num2str(subjectName) ' Subject count:' num2str(subjNum)])
    ...
    for runNum = 1:numTaskRunsPerSubj
        ...
        for trialNum = 1:numTrialsPerRun
            ...
            for encodingTR = 1:numTRsForEncoding
                if encodingTR == 1
                    reffunc{currentLine} = [trialType '_Task_Encoding_' logicCue '_' semanticCue '_' responseCue '_' taskID];
                else
                    reffunc{currentLine} = [trialType '_Task_Enc_' logicCue '_' semanticCue '_' responseCue '_' taskID];
                end
            ...
            if rem(str2num(firstJitterEDATA{currentLine_EDAT}),TRvalue)==0
                ...
                for jitter1TR = 1:firstJitterTRs
                    if jitter1TR == 1
                        reffunc{currentLine} = [trialType '_' logicCue '_' semanticCue '_' responseCue '_' taskID '_StartRest'];
                    else
                        reffunc{currentLine} = [trialType '_' logicCue '_' semanticCue '_' responseCue '_' taskID '_Rest'];
                    end
                    numTRsLeftForTrial = numTRsLeftForTrial - 1;
                    msCount = msCount + TRvalue;
                    msOffsetList(currentLine) = (currentLine * TRvalue) - msCount;
            ...
            for probeNum=1:numProbesPerTrial
                ...
                for probeTR = 1:numTRsPerProbe
                    if strcmp(probeAccuracy, 'Wrong')
                        reffunc{currentLine} = [trialType '_Task_Probe' num2str(probeNum) '_Wrong_' logicCue '_' semanticCue '_' responseCue '_' taskID];
                    else
                        reffunc{currentLine} = [trialType '_Task_Probe' num2str(probeNum) '_Correct_' logicCue '_' semanticCue '_' responseCue '_' taskID];
                ...
                if probeNum ~= numProbesPerTrial
                    for probeDelayTR = 1:numTRsForProbeDelay
                        reffunc{currentLine} = [trialType '_' logicCue '_' semanticCue '_' responseCue '_' taskID '_ProbeRest'];
                        numTRsLeftForTrial = numTRsLeftForTrial -1;
                        msOffsetList(currentLine) = (currentLine * TRvalue) - msCount;
                        currentLine = currentLine + 1;
                    end
            ...
            if rem(str2num(secondJitterEDATA{currentLine_EDAT}),TRvalue)==0
                ...
                for jitter2TR = 1:secondJitterTRs
                    if jitter2TR == secondJitterTRs
                        %reffunc{currentLine} = [trialType '_' logicCue '_' semanticCue '_' responseCue '_' taskID '_EndRest'];
        ...
        if ~isempty(find(abs(msOffsetList) > 1))
            disp(['Warning: TR offsets are occasionally greater than 1 for reference function. (msCounts = ' num2str(msCount) ')']);
        end
