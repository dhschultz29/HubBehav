...
runstart=load(['runstart.1D']);

for subjNum=1:length(subjNumStr);
    filename=subjNumStr{subjNum,1};
    data=load ([num2str(filename) '_Task_FD.1D']);
    temp=data<0.5;
    temp2=double(temp);
    scrubbed_task=ones(4648,1);
    ...
    scrubbed_task_final=(scrubbed_task(1:4648,:));
    scrubbed_task_final2=(scrubbed_task_final.*(runstart));
    dlmwrite(([num2str(filename) '_Task_censor.1D']),scrubbed_task_final2);
    filename
