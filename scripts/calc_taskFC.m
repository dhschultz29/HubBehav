...
All_subj_all_task_FC_r=zeros(360,360,64,100);
All_subj_all_task_FC_p=zeros(360,360,64,100);
...
for subjNum=1:100
    ...
    for taskNum=1:64
        ...
        All_subj_all_task_FC_r(:,:,taskNum,subjNum)=TaskFC_corr_r;
        All_subj_all_task_FC_p(:,:,taskNum,subjNum)=TaskFC_corr_p;
        clear tasktimepoints_bytask
    end
end
       
All_subj_all_rest_FC_r=zeros(360,360,100);
All_subj_all_rest_FC_p=zeros(360,360,100);
        
for subjNum=1:100
    subjNum
        [RestFC_corr_r,RestFC_corr_p]=corrcoef(output.restGLMVars{subjNum}.fMRI_resids');
        All_subj_all_rest_FC_r(:,:,subjNum)=RestFC_corr_r;
        All_subj_all_rest_FC_p(:,:,subjNum)=RestFC_corr_p;
end        
