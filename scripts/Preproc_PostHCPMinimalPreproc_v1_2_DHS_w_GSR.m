function [output] = Preproc_PostHCPMinimalPreproc_v1_2_DHS_w_GSR(SUBJECTLIST)
...
%1.2: Added check for GLM design matrix deficiency. FD is now temporally filtered (0.3Hz lowpass; consistent with Siegel et al. 2016). Scrubbing now removes 1 before and 2 after a high motion event (consistent with Power et al. 2012).
...
FDTHRESH=0.3;  %Framewise displacement; The threshold (in millimeters) for flagging in-scanner movement
...
outputdatadir='/projects/IndivRITL/data/results/HubBehav/';
...
for subjIndex=1:numSubjs
    ...
    SUBJDIROUTPUT=['/projects/IndivRITL/data/results/HubBehav/' subjNum '/'];   %Typically set to be same as subjDir
    ...
    if and(~isempty(TASKRUNS), or(RERUN_TASKGLM == 1, ~exist(savedfile, 'file')))
        ...
        %%Parameters
        GSR=1;      %GSR = 0 if no GSR, 1 if you want to include GSR
        NPROC=4;    %Number of processors to use for GLM
        %Specify the number of nuisance regressors
        NUMREGRESSORS_NUISANCE=16;
        %Add 2 regressors for GSR
